...
const path = "C:/Users/julio/OneDrive/Escritorio/S18 Crear.xlsx";
...
async function getActualMembers() {
  const res = await axios.get(`${process.env.LOCALHOST}teams/actualselection`, {
    headers: { token: `Bearer ${token}` },
  });
  teams = res.data.getActualselTeams;
  ...
  try {
    ...
    for (const d of data) {
      ...
      if (member && team) {
        ...
        //llamo a la api
        await axios.put(
          `${process.env.LOCALHOST}teams/selection/${team._id}`, // (putMemberTeam)
          { members: member._id },
          {
            headers: { token: `Bearer ${token}` },
          }
