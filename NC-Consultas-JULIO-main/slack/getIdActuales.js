const dotenv = require("dotenv");
dotenv.config();
//Slack auth
// Your OAuth token
const { WebClient } = require("@slack/web-api");
const slackToken = process.env.SLACK_TOKEN;
const web = new WebClient(slackToken);

//configuraciones generales
const axios = require("axios");
const fs = require("fs");
const { channel } = require("diagnostics_channel");
const reader = require("xlsx");
let token = process.env.TOKEN;

const path = "C:/Users/julio/OneDrive/Escritorio/slackmembers.xlsx";

const file = reader.readFile(path);
const sheets = file.SheetNames;
let slackMembers = [];

for (let i = 0; i < sheets.length; i++) {
  const temp = reader.utils.sheet_to_json(file.Sheets[file.SheetNames[i]]);
  temp.forEach((res) => {
    slackMembers.push(res);
  });
}

let output = [];

console.log(slackMembers.length);

async function getMembers() {
  try {
    const res = await axios.get(`${process.env.LOCALHOST}members/actualfull`, {
      headers: { token: `Bearer ${token}` },
    });
    const fullMembers = res.data.getActualFullMembers;

    console.log(fullMembers.length);

    fullMembers.forEach((member) => {
      const encontrado = slackMembers.find(
        (slack) => slack.email.toLowerCase() == member.email.toLowerCase()
      );
      if (encontrado) {
        // output.push(member.email.toLowerCase() + )+
        // console.log(encontrado.email + "  " + encontrado.userid);
      } else {
        console.log(member.email); // + "  " + "NOT FOUND");
        // console.log(member.email + "  " + "NOT FOUND");
      }
    });
  } catch (error) {
    console.error(error);
  }
}

getMembers();
