...
async function getTeams() {
  ...
  const res = await axios.get(
    ...
    `https://nocountryback.herokuapp.com/api/teams`,
    {
  ...
  if (allTeams.length > 0) {
    teamsNormalized.push("Team;fullname;email");
  }
  allTeams.map((team) => {
    team.members.forEach((member) => {
      ...
      teamsNormalized.push(
        team.name.toLowerCase() + ";" + member.fullname + ";" + member.email
      );
    });
  ...
  if (teamsNormalized.length > 0) {
    ...
    fs.writeFileSync("Equipos IDX asd.txt", fullnamesWithSpace);
  }
}
// teamsC3.map((team) => {
//     team.members.forEach((member) => {
